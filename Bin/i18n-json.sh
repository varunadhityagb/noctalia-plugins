...
FOLDER_PATH=""  # Set dynamically per plugin
...
# Function to find all plugins with i18n directories
find_plugins_with_i18n() {
    find "$PLUGINS_DIR" -maxdepth 2 -type d -name "i18n" -exec dirname {} \; | sort
}

# Function to get all unique language codes across all plugins (excluding en.json)
get_all_languages() {
    find "$PLUGINS_DIR" -path "*/i18n/*.json" -exec basename {} \; | \
        sed 's/\.json$//' | \
        grep -v "^en$" | \
        sort -u
}

# Function to generate global report header
generate_global_header() {
    local timestamp=$(date '+%Y-%m-%d %H:%M:%S')

    echo "================================================================================"
    echo "               PLUGIN LANGUAGE FILE COMPARISON REPORT"
    echo "================================================================================"
    ...
    echo "and identifies missing keys and extra keys in each language for each plugin."
    echo ""
}

# Function to generate plugin section header
generate_plugin_header() {
    local plugin_name=$1
    local plugin_path=$2
...
# Global counter for processed files (used by process_plugin)
PLUGIN_PROCESSED_COUNT=0

# Process a single plugin's i18n directory
# Sets PLUGIN_PROCESSED_COUNT to number of language files processed
process_plugin() {
    local plugin_path="$1"
    local target_language="$2"
    local plugin_name=$(basename "$plugin_path")

    FOLDER_PATH="$plugin_path/i18n"
    PLUGIN_PROCESSED_COUNT=0
    ...
    if [[ ! -f "$ref_file_path" ]]; then
        print_color $RED "Error: Reference file '$ref_file_path' does not exist in plugin $plugin_name" >&2
        return 1
    fi

    # Generate plugin header
    generate_plugin_header "$plugin_name" "$plugin_path"
    ...
    if [[ -n "$target_language" ]]; then
        ...
        if [[ ! -f "$target_file" ]]; then
            if $TRANSLATE_MODE; then
                print_color $BLUE "Creating new language file: $target_file" >&2
                echo '{}' > "$target_file"
            else
                print_color $YELLOW "Warning: Language file '$target_file' does not exist in plugin $plugin_name. Skipping..." >&2
                rm -f "$ref_keys_file"
                return 0
            fi
        fi
        ...
        # All languages mode
        if $TRANSLATE_MODE; then
            # In translate mode: use union of all languages across all plugins
            while IFS= read -r lang; do
                local lang_file="$FOLDER_PATH/${lang}.json"
                # Create file if it doesn't exist
                if [[ ! -f "$lang_file" ]]; then
                    print_color $BLUE "Creating new language file: $lang_file" >&2
                    echo '{}' > "$lang_file"
                fi
                language_files+=("$lang_file")
            done < <(get_all_languages)
        else
            # Non-translate mode: only process existing files
    ...
    for lang_file in "${language_files[@]}"; do
        ...
        if compare_language "$lang_file" "$lang_name" "$ref_keys_file"; then
            PLUGIN_PROCESSED_COUNT=$((PLUGIN_PROCESSED_COUNT + 1))
        else
...
# Main function - iterates through all plugins
main() {
    local target_language="$1"

    print_color $BLUE "Starting plugin language file comparison..." >&2

    # Check dependencies
    check_dependencies

    # Find all plugins with i18n directories
    local -a plugins
    while IFS= read -r plugin_dir; do
        plugins+=("$plugin_dir")
    done < <(find_plugins_with_i18n)

    if [[ ${#plugins[@]} -eq 0 ]]; then
        print_color $RED "Error: No plugins with i18n directories found in $PLUGINS_DIR" >&2
        exit 1
    fi

    print_color $GREEN "Found ${#plugins[@]} plugin(s) with i18n directories" >&2
    echo "" >&2

    # Generate global report header
    generate_global_header
    ...
    for plugin_dir in "${plugins[@]}"; do
        ...
        print_color $BLUE "Processing plugin: $plugin_name" >&2

        if process_plugin "$plugin_dir" "$target_language"; then
            total_processed=$((total_processed + PLUGIN_PROCESSED_COUNT))
    ...
    echo "GLOBAL SUMMARY"
    echo "================================================================================"
    echo "Total plugins processed: $plugins_processed"
    echo "Total language files processed: $total_processed"
    ...
    print_color $GREEN "Comparison completed: Processed $total_processed language files across $plugins_processed plugins" >&2
}
...
show_usage() {
    ...
    echo "" >&2
    echo "This script iterates through all plugins and compares JSON language files" >&2
    echo "in each plugin's i18n/ directory against the English reference." >&2
    echo "" >&2
    echo "Arguments:" >&2
    ...
    echo "  - Plugins directory: $PLUGINS_DIR" >&2
    echo "  - Each plugin's i18n/ subdirectory is processed" >&2
    echo "  - Reference file: $REFERENCE_FILE" >&2
    echo "" >&2
    echo "Examples:" >&2
    echo "  $0                    # Compare all languages in all plugins" >&2
    echo "  $0 fr                 # Compare only French (fr.json) in all plugins" >&2
    ...
    echo "  $0 --translate fr     # Translate, clean, and sort French only in all plugins" >&2
    echo "" >&2
    echo "Requirements:" >&2
    echo "  - jq must be installed" >&2
    echo "  - curl must be installed (for --translate mode)" >&2
    echo "  - $REFERENCE_FILE must exist in each plugin's i18n/ directory" >&2
